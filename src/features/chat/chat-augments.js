...
function allowedToEmbed(url) {
    const trusted = TRUSTED_HOSTS.includes(url.host) || isTrustedOrigin(url.origin);
...
const embedScrollCallbacks = new WeakMap();
const pendingEmbedPrompts = new WeakSet();
let embedEventsInstalled = false;
function installEmbedEvents() {
    if (embedEventsInstalled) return;
    embedEventsInstalled = true;
    document.addEventListener('click', event => {
        const prompt = event.target.closest?.('[data-lce-embed-url]');
        if (!prompt || pendingEmbedPrompts.has(prompt)) return;
        event.preventDefault(); event.stopPropagation();
        const url = parseUrl(prompt.dataset.lceEmbedUrl);
        if (!url) return;
        pendingEmbedPrompts.add(prompt);
        requestOriginTrust(url.origin, T('domain_image'), { persistent: true }).then(action => {
            if (action !== 'submit' && action !== 'always') return;
            const link = prompt.previousElementSibling;
            if (!link || link.tagName !== 'A' || link.href !== url.href) return;
            const image = document.createElement('img');
            image.src = url.href; image.alt = url.href; image.classList.add('lce-img');
            image.onload = embedScrollCallbacks.get(prompt) || (() => {});
            link.classList.add('lce-img-link'); link.replaceChildren(image);
            prompt.remove();
        }).catch(error => console.warn('🐈‍⬛ [LCE] 圖片來源確認失敗:', error))
            .finally(() => { pendingEmbedPrompts.delete(prompt); });
    });
}
            ...
            if (url) {
                ...
                if (embedType === EMBED.Image) {
                    ...
                    if (embedType !== EMBED.None) {
                        ...
                        promptTrust.dataset.lceEmbedUrl = url.href;
                        embedScrollCallbacks.set(promptTrust, scrollToEnd);
...
/** 處理新增訊息；pending 訊息等移除 class 後再處理，不能提前標記完成。 */
const processedText = new WeakMap();
function processChatMessage(msgEl) {
    if (!msgEl) return;
    if (msgEl.getAttribute(HANDLED_ATTR) === 'true' && (!processedText.has(msgEl) || processedText.get(msgEl) === msgEl.textContent)) {
        processedText.set(msgEl, msgEl.textContent);
        return;
    }
    if (!MSG_CLASSES.some(c => msgEl.classList.contains(c)) || msgEl.classList.contains('lce-pending')) return;
    const contentSpan = msgEl.querySelector('.chat-room-message-content');
    const scrolledToEnd = ElementIsScrolledToEnd(CHATLOG);
    processChatAugmentsForLine(contentSpan || msgEl,
        () => { if (scrolledToEnd) ElementScrollToEnd(CHATLOG); }, !!contentSpan);
